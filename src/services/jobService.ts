...
export async function listJobs(userId: string, status?: JobStatus): Promise<JobDTO[]> {
...
export async function getJobById(userId: string, jobId: string): Promise<JobDTO> {
...
export async function createJob(userId: string, input: CreateJobInput): Promise<JobDTO> {
...
export async function updateJob(userId: string, jobId: string, input: UpdateJobInput): Promise<JobDTO> {
...
export async function updateJobStatus(
  ...
  try {
    ...
    const targetCount = await JobModel.countDocuments({ userId, status: input.status }).session(session)
    const clampedOrder = Math.max(0, Math.min(input.order, Math.max(0, targetCount)))
...
export async function deleteJob(userId: string, jobId: string): Promise<void> {
  const deleted = await JobModel.findOneAndDelete({ _id: jobId, userId }).lean<{ _id: unknown }>()
...
export async function getStatsSummary(userId: string): Promise<JobStatsSummary> {
